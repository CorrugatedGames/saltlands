...
public static class WorldVisualizer
{
    ...
    public static void RenderWorld(SaltLandsWorld world)
    {
        ...
        int smallestX = (int)world.Generator.ChunkGenerator.LoadedChunks.OrderBy(chunk => chunk.Position.X).First().Position.X;
        int smallestY = (int)world.Generator.ChunkGenerator.LoadedChunks.OrderBy(chunk => chunk.Position.Y).First().Position.Y;
        int largestX = (int)world.Generator.ChunkGenerator.LoadedChunks.OrderByDescending(chunk => chunk.Position.X).First().Position.X;
        int largestY = (int)world.Generator.ChunkGenerator.LoadedChunks.OrderByDescending(chunk => chunk.Position.Y).First().Position.Y;
        ...
        foreach (var chunk in world.Generator.ChunkGenerator.LoadedChunks)
        {
